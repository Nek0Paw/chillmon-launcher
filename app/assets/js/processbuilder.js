...
class ProcessBuilder {
    ...
    build(){
        ...
        // Prefer java.exe so stdout works and early crashes are visible.
        if (process.platform === 'win32' && typeof javaExec === 'string' && /javaw\.exe$/i.test(javaExec)) {
            const asJava = javaExec.replace(/javaw\.exe$/i, 'java.exe')
            if (fs.existsSync(asJava)) {
                javaExec = asJava
            }
        }
                ...
                if(argDiscovery.test(args[i])){
                    ...
                    switch(identifier){
                        ...
                        case 'natives_directory':
                            ...
                            break
                        case 'launcher_name':
                            val = args[i].replace(argDiscovery, 'Chillmon')
                            break
