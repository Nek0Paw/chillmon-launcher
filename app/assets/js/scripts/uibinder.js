...
const { DistroAPI, loadLocalDistribution } = require('./assets/js/distromanager')
...
function logBootError(err) {
    try {
        const fs = require('fs-extra')
        const path = require('path')
        const line = `[${new Date().toISOString()}] boot error ${err && (err.stack || err.message || err)}\n`
        fs.appendFileSync(path.join(ConfigManager.getLauncherDirectory(), 'distro-boot.log'), line)
    } catch {
        // ignore
    }
}
...
async function bootLauncher() {
    ...
    uiBooted = true
    try {
        const data = await loadLocalDistribution()
        ...
        console.error('Failed to boot launcher', err)
        logBootError(err)
        uiBooted = false
...
ipcRenderer.on('distributionIndexDone', () => {
    scheduleBoot()
})

scheduleBoot()
setTimeout(() => {
    if (!uiBooted && !fatalStartupError) {
        scheduleBoot()
    }
}, 1500)
